...
const log = createLogger('NotificacoesPopover');
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          log.debug("Nova notificação recebida:", payload);
          fetchNotificacoes();
        }
      )
      .on(
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <Bell className={`h-5 w-5 transition-transform ${hasNewNotification ? 'animate-bounce text-accent' : 'text-primary-foreground'}`} />
          {naoLidas > 0 && (
            <Badge
              variant="destructive"
