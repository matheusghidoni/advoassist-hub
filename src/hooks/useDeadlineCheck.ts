...
const log = createLogger('useDeadlineCheck');
...
export const useDeadlineCheck = () => {
  ...
  useEffect(() => {
    const checkDeadlines = async () => {
      ...
      if (lastCheck === today) {
        log.debug('Deadline check already done today');
        return;
      }
      ...
      try {
        log.debug('Checking deadlines automatically...');

        const { data, error } = await supabase.functions.invoke('check-deadline-notifications');

        if (error) {
          log.error('Error checking deadlines:', error);
          return;
        }

        log.debug('Deadline check result:', data);
        ...
        log.error('Error in deadline check:', err);
      }
    };
