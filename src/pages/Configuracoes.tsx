...
import { User, Building2, Bell, Lock, BellRing, RefreshCw, MessageCircle } from "lucide-react";
...
export default function Configuracoes() {
  ...
  const handleCheckDeadlines = async () => {
    ...
    try {
      ...
      log.error("Erro ao verificar prazos:", error);
  ...
  const loadProfile = async () => {
    try {
      ...
      if (notifError && notifError.code !== 'PGRST116') {
        log.error('Erro ao carregar preferências de notificação:', notifError);
      }
      ...
      if (notifData) {
        ...
        if (createError) {
          log.error('Erro ao criar preferências de notificação:', createError);
        } else if (newPrefs) {
  ...
  const handleUpdateOffice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      log.error('Erro ao atualizar escritório:', error);
  ...
  const handleUpdateNotificationPreference = async (field: string, value: boolean) => {
    try {
      ...
      log.error('Erro ao atualizar preferência:', error);
